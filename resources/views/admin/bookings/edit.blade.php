        ...
        <div class="card shadow">
            <div class="card-header">
                <div class="d-sm-flex align-items-center justify-content-between mb-4">
                    ...
                    <a href="{{ route('admin.bookings.index') }}" class="btn btn-secondary btn-sm shadow-sm">{{ __('Go Back') }}</a>
                </div>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.bookings.update', $booking->id) }}" method="POST">
                    ...
                    <div class="form-group">
                        <label for="arena_id">{{ __('Lapangan') }}</label>
                        <select name="arena_id" id="arena_id" class="form-control">
                            @foreach($arenas as $arena)
                                <option {{ $booking->arena->lapangan == $arena->lapangan ? 'selected' : null }} value="{{ $arena->id }}">{{ $arena->lapangan }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="form-group">
